...
from backend_mcp.auth.service import get_user_from_session
from backend_mcp.core.config import get_settings
from backend_mcp.db.clients import get_redis_client
...
async def get_current_user_optional(
    ...
    if token is None:
        return None
    return await get_user_from_session(token)


async def validate_internal_api_key(x_internal_api_key: str | None = Header(default=None)) -> None:
...
async def rate_limit_tools(request: Request) -> None:
    settings = get_settings()
    await _enforce_rate_limit(request, bucket="tools", limit=settings.tool_rate_limit_per_minute)
